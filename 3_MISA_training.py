'''
build the integrated diagnostic model M_ISA:
using the short, long diameters, diamter ratio, and adc value combined with the img predictions
'''
...
def get_WISDOM_model(shape_p):
    ...
    WISDOM_model_MISA = keras.Model(inputs=model_input, outputs=[output_meta_status, output_meta_ratio],
                            name="WISDOM_model_M_ISA")

    print('WISDOM_model_M_ISA')
    WISDOM_model_MISA.summary()

    return WISDOM_model_MISA
...
if __name__ == '__main__':
    ...
    feas_num = 5 #include the size features, adc and img prediction

    center_name = 'CenterI'

    basic_model_name = 'MISA'
    ...
    patches_adc = pickle.load(open(
        ...
        "rb"))
    patches_adc = patches_feas_zero_padding(patches_adc, patch_num)
