'''
step 2
read the 2D bounding box containing lymph node and save in bin files, with z-max normalization [0, 1],
and save the image data in three-channel to cooperate with the pre-trained model,
also save the corresponding year, name, and label information for each patient,
convinent for the model building process
date: 2021-02-06
author: xiav
'''
...
if __name__ == '__main__':
    ...
    modality = 'T2'
    center_name = 'FUDAN' # SHANXI FUDAN YUNNAN
    ...
    for index in range(1,  all_num+1):
        ...
        vol_name = modality + '_' + p_name + '_hm.nii.gz'

        p_vol_path = path_ori + '\\' + vol_name
        p_vol = sitk.ReadImage(p_vol_path)
        p_vol_array = sitk.GetArrayFromImage(p_vol)
        p_vol_array_std = img_std_zmax(p_vol_array)

        # # repeat as three channels
        p_vol_array_std_3channel = np.repeat(p_vol_array_std[..., np.newaxis], 3, 3)

        # p_temp = p_vol_array_std_3channel[0, :]
        # plt.subplot(131)
        # plt.imshow(p_vol_array_std_3channel[0, :], cmap='gray')
        # plt.subplot(132)
        # plt.imshow(p_vol_array_std_3channel[1, :], cmap='gray')
        # plt.subplot(133)
        # plt.imshow(p_vol_array_std_3channel[2, :], cmap='gray')
        # plt.show()
